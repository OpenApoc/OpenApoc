		...
		const InitialGameStateExtractor &e)
	{
		...
		// Standart animations
		{
			...
			p->standart_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::None,
				...
				e.getAnimationEntry(dataAD, dataUA, dataUF, 0, { x, y },100, 1, true, false, false, getDownOffset(x,y), getDownOffset(x, y));

			// Dead state: 4
			p->standart_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::None,
				BodyState::Dead}][{x, y}] =
				e.getAnimationEntry(dataAD, dataUA, dataUF, 4, { x, y }, getDownOffset(x, y), getDownOffset(x, y));

			// Prone static/aiming: 0 first frame
			p->standart_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::None,
				BodyState::Prone}][{x, y}] =
				e.getAnimationEntry(dataAD, dataUA, dataUF, 0, { x, y }, 100, 1, true, false, false, getProneOffset(x, y), getProneOffset(x, y));
			...
			p->standart_animations[{ItemWieldMode::None, HandState::Firing, MovementState::None,
				BodyState::Prone}][{x, y}] =
				e.getAnimationEntry(dataAD, dataUA, dataUF, 6, { x, y }, getProneOffset(x, y), getProneOffset(x, y));
			...
			p->standart_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::Normal,
				BodyState::Prone}][{x, y}] =
				e.getAnimationEntry(dataAD, dataUA, dataUF, 0, { x, y }, pFrames, false, getProneOffset(x, y), getProneOffset(x, y));
		}
		...
		// Body state change animations
		{
			...
			p->body_state_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::None,
				...
				e.getAnimationEntry(dataAD, dataUA, dataUF, 2, { x, y }, getProneOffset(x, y), getProneOffset(x, y));

			// Body Prone -> Kneeling 1  (offset the last unkneeling frame)
			auto kneelingFrames = e.getAnimationEntry(dataAD, dataUA, dataUF, 1, { x, y }, getProneOffset(x, y), getProneOffset(x, y));
			kneelingFrames->frames[kneelingFrames->frame_count - 1].unit_image_parts[BattleUnitAnimationPack::AnimationEntry::Frame::UnitImagePart::Body].offset = -getLastFrameOffset(x, y);
			p->body_state_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::None,
			...
			p->body_state_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::None,
				...
				e.getAnimationEntry(dataAD, dataUA, dataUF, 3, { x, y }, getProneOffset(x, y), getProneOffset(x, y));

			// Body Kneel -> Dead animation: 2+3
			p->body_state_animations[{ItemWieldMode::None, HandState::AtEase, MovementState::None,
				BodyState::Kneeling, BodyState::Dead}][{x, y}] = e.combineAnimationEntries(
					e.getAnimationEntry(dataAD, dataUA, dataUF, 2, { x, y }, getProneOffset(x, y), getProneOffset(x, y)),
					e.getAnimationEntry(dataAD, dataUA, dataUF, 3, { x, y }, getProneOffset(x, y), getProneOffset(x, y)));
		}
	}

	void InitialGameStateExtractor::extractAnimationPackMulti(sp<BattleUnitAnimationPack> p,
		const std::vector<AnimationDataAD> &dataAD,
		const std::vector<AnimationDataUA> &dataUA,
