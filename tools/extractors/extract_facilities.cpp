...
void InitialGameStateExtractor::extractFacilities(GameState &state) const
{
	...
	LogInfo("Number of facility strings: {0}", (unsigned)data.facility_names->count());
	LogInfo("Number of facility data chunks: {0}", (unsigned)data.facility_data->count());
	...
	for (unsigned i = 2; i < data.facility_names->count(); i++)
	{
		...
		LogInfo("Facility {0}: {1} cost {2} image_offset {3} size {4} build_time {5} maint {6} "
		        "capacity {7}",
		        i, id, (int)f.cost, (int)f.image_offset, (int)f.size, (int)f.build_time,
		        (int)f.maintainance_cost, (int)f.capacity);
		LogInfo("u1 {0:04x} u2 {1:04x}", (unsigned)f.unknown1, (unsigned)f.unknown2);
		...
		facilityType->sprite = fw().data->loadImage(
		    format("PCK:xcom3/ufodata/base.pck:xcom3/ufodata/base.tab:{0}:xcom3/ufodata/base.pcx",
		           (int)f.image_offset));
