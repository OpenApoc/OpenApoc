...
void fillAgentImagePacksByDefault(GameState &state, sp<AgentType> a, UString imagePackName)
{
	a->image_packs[a->image_packs.size() - 1][BodyPart::Body] = {
	    &state, fmt::format("{}{}{}", BattleUnitImagePack::getPrefix(), imagePackName, "a")};
	a->image_packs[a->image_packs.size() - 1][BodyPart::Legs] = {
	    &state, fmt::format("{}{}{}", BattleUnitImagePack::getPrefix(), imagePackName, "b")};
	a->image_packs[a->image_packs.size() - 1][BodyPart::Helmet] = {
	    &state, fmt::format("{}{}{}", BattleUnitImagePack::getPrefix(), imagePackName, "c")};
	a->image_packs[a->image_packs.size() - 1][BodyPart::LeftArm] = {
	    &state, fmt::format("{}{}{}", BattleUnitImagePack::getPrefix(), imagePackName, "d")};
	a->image_packs[a->image_packs.size() - 1][BodyPart::RightArm] = {
	    &state, fmt::format("{}{}{}", BattleUnitImagePack::getPrefix(), imagePackName, "e")};
}
...
void InitialGameStateExtractor::extractAgentTypes(GameState &state) const
{
	...
	for (unsigned i = 1; i < data_u.agent_types->count(); i++)
	{
		...
		UString id = fmt::format("{}{}", AgentType::getPrefix(), canon_string(a->name));
		...
		switch (i)
		{
			...
			case UNIT_TYPE_LOWER_CLASS_MALE_3:
				if (i == UNIT_TYPE_ANDROID)
				{
					...
					a->animation_packs.emplace_back(
					    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "civ"));
					a->animation_packs.emplace_back(
					    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "civ"));
					a->animation_packs.emplace_back(
					    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "civ"));
			...
			case UNIT_TYPE_MULTIWORM_EGG:
				...
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "mwegg1"));
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "mwegg2"));
			...
			case UNIT_TYPE_CHRYSALIS:
				a->appearance_count = 2;
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "chrys1"));
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "chrys2"));
			...
			case UNIT_TYPE_QUEENSPAWN:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "queen"));
			...
			case UNIT_TYPE_BRAINSUCKER:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}bsk", BattleUnitAnimationPack::getPrefix()));
			...
			case UNIT_TYPE_HYPERWORM:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}hypr", BattleUnitAnimationPack::getPrefix()));
			...
			case UNIT_TYPE_SPITTER:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}spitr", BattleUnitAnimationPack::getPrefix()));
			...
			case UNIT_TYPE_POPPER:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}popper", BattleUnitAnimationPack::getPrefix()));
				bodyTypeName = "POPPER";
				infiltrationID = 8;
				a->growthChance = 2;
				a->detectionWeight = 2;
			...
			case UNIT_TYPE_MICRONOID:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}micro", BattleUnitAnimationPack::getPrefix()));
			...
			case UNIT_TYPE_MULTIWORM:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "multi"));
			...
			case UNIT_TYPE_MEGASPAWN:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "mega"));
			...
			case UNIT_TYPE_PSIMORPH:
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "psi"));
			...
			default:
				...
				if (i == UNIT_TYPE_GANGSTER)
				{
					a->appearance_count = 2;
					a->animation_packs.emplace_back(
					    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "unit"));
				}
				a->animation_packs.emplace_back(
				    &state, fmt::format("{}{}", BattleUnitAnimationPack::getPrefix(), "unit"));
				break;
		...
		a->bodyType = {&state,
		               fmt::format("{}{}", AgentBodyType::getPrefix(), canon_string(bodyTypeName))};
		...
		switch (i)
		{
			...
			case UNIT_TYPE_BRAINSUCKER:
				a->shadow_pack = {&state,
				                  fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "bsks")};
				break;
			case UNIT_TYPE_HYPERWORM:
				a->shadow_pack = {&state,
				                  fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "hyprs")};
				break;
			case UNIT_TYPE_SPITTER:
				a->shadow_pack = {&state,
				                  fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "spitrs")};
				break;
			case UNIT_TYPE_POPPER:
				a->shadow_pack = {&state,
				                  fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "poppers")};
				break;
			case UNIT_TYPE_MEGASPAWN:
				a->shadow_pack = {&state,
				                  fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "megas")};
				break;
			case UNIT_TYPE_PSIMORPH:
				a->shadow_pack = {&state,
				                  fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "psis")};
				break;
			...
			default:
				a->shadow_pack = {&state,
				                  fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "shadow")};
				break;
		}
		...
		switch (i)
		{
			...
			case UNIT_TYPE_QUANTUM_PHYSIST:
				...
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "scntst")};
				break;
			case UNIT_TYPE_GANG_LEADER:
			...
			case UNIT_TYPE_CORPORATE_BOSS:
				...
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "rm1")};
				break;
			case UNIT_TYPE_CULT_LEADER:
			...
			case UNIT_TYPE_ANDROID:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robot")};
				a->image_packs[0][BodyPart::Helmet] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robo1")};
				a->image_packs.push_back(std::map<BodyPart, StateRef<BattleUnitImagePack>>());
				a->image_packs[1][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robot")};
				a->image_packs[1][BodyPart::Helmet] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robo2")};
				a->image_packs.push_back(std::map<BodyPart, StateRef<BattleUnitImagePack>>());
				a->image_packs[2][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robot")};
				a->image_packs[2][BodyPart::Helmet] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robo3")};
				a->image_packs.push_back(std::map<BodyPart, StateRef<BattleUnitImagePack>>());
				a->image_packs[3][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robot")};
				a->image_packs[3][BodyPart::Helmet] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "robo4")};
				break;
			case UNIT_TYPE_GREY:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "grey")};
				break;
			case UNIT_TYPE_UPPER_CLASS_FEMALE_1:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "rw1")};
				break;
			case UNIT_TYPE_UPPER_CLASS_FEMALE_2:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "rw2")};
				break;
			case UNIT_TYPE_UPPER_CLASS_FEMALE_3:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "rw3")};
				break;
			case UNIT_TYPE_UPPER_CLASS_MALE_1:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "rm1")};
			...
			case UNIT_TYPE_UPPER_CLASS_MALE_2:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "rm2")};
				break;
			case UNIT_TYPE_UPPER_CLASS_MALE_3:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "rm3")};
				break;
			case UNIT_TYPE_CIVILIAN_FEMALE_1:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "nw1")};
				break;
			case UNIT_TYPE_CIVILIAN_FEMALE_2:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "nw2")};
				break;
			case UNIT_TYPE_CIVILIAN_FEMALE_3:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "nw3")};
				break;
			case UNIT_TYPE_CIVILIAN_MALE_1:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "nm1")};
				break;
			case UNIT_TYPE_CIVILIAN_MALE_2:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "nm2")};
				break;
			case UNIT_TYPE_CIVILIAN_MALE_3:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "nm3")};
				break;
			case UNIT_TYPE_LOWER_CLASS_MALE_1:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "sm1")};
				break;
			case UNIT_TYPE_LOWER_CLASS_MALE_2:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "sm2")};
				break;
			case UNIT_TYPE_LOWER_CLASS_MALE_3:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "sm3")};
				break;
			case UNIT_TYPE_LOWER_CLASS_FEMALE_1:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "sw1")};
				break;
			case UNIT_TYPE_LOWER_CLASS_FEMALE_2:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "sw2")};
				break;
			case UNIT_TYPE_LOWER_CLASS_FEMALE_3:
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "sw3")};
				break;
			case UNIT_TYPE_MULTIWORM_EGG:
				a->inventoryBackground =
				    fw().data->loadImage("xcom3/tacdata/equippic/mwormegg.pcx");
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "mwegga")};
				a->image_packs[0][BodyPart::Helmet] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "mweggb")};
				a->image_packs.push_back(std::map<BodyPart, StateRef<BattleUnitImagePack>>());
				a->image_packs[1][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "mwegga")};
				a->image_packs[1][BodyPart::Helmet] = {
			...
			case UNIT_TYPE_BRAINSUCKER:
				...
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "bsk")};
				break;
			case UNIT_TYPE_MULTIWORM:
				a->inventoryBackground = fw().data->loadImage("xcom3/tacdata/equippic/mworm.pcx");
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "multi")};
				break;
			case UNIT_TYPE_HYPERWORM:
				a->inventoryBackground = fw().data->loadImage("xcom3/tacdata/equippic/hyperwm.pcx");
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "hypr")};
				break;
			case UNIT_TYPE_CHRYSALIS:
				a->inventoryBackground =
				    fw().data->loadImage("xcom3/tacdata/equippic/chrysali.pcx");
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "chrysa")};
				a->image_packs[0][BodyPart::Helmet] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "chrysb")};
				a->image_packs.push_back(std::map<BodyPart, StateRef<BattleUnitImagePack>>());
				a->image_packs[1][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "chrysa")};
				a->image_packs[1][BodyPart::Helmet] = {
			...
			case UNIT_TYPE_SPITTER:
				...
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "spitr")};
				break;
			case UNIT_TYPE_POPPER:
				a->inventoryBackground = fw().data->loadImage("xcom3/tacdata/equippic/popper.pcx");
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "popper")};
				break;
			case UNIT_TYPE_MEGASPAWN:
				a->inventoryBackground =
			...
			case UNIT_TYPE_PSIMORPH:
				...
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "psi")};
				break;
			case UNIT_TYPE_QUEENSPAWN:
				a->inventoryBackground = fw().data->loadImage("xcom3/tacdata/equippic/queen.pcx");
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "queena")};
				a->image_packs[0][BodyPart::Legs] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "queenb")};
				break;
			case UNIT_TYPE_MICRONOID:
				a->inventoryBackground =
				    fw().data->loadImage("xcom3/tacdata/equippic/agregate.pcx");
				a->image_packs[0][BodyPart::Body] = {
				    &state, fmt::format("{}{}", BattleUnitImagePack::getPrefix(), "micro")};
				break;
		}
			...
			case UNIT_TYPE_MICRONOID:
				if (walkSfxNumbered)
					for (int i = 1; i <= walkSfxCount; i++)
						a->walkSfx.push_back(fw().data->loadSample(fmt::format(
						    "RAWSOUND:xcom3/rawsound/tactical/aliens/movemnts/{}{}.raw:22050",
						    walkSfxName, i)));
				else
					a->walkSfx.push_back(fw().data->loadSample(
					    fmt::format("RAWSOUND:xcom3/rawsound/tactical/aliens/movemnts/{}.raw:22050",
					                walkSfxName)));
				if (crySfxNumbered)
					for (int i = 1; i <= crySfxCount; i++)
						a->crySfx.push_back(fw().data->loadSample(fmt::format(
						    "RAWSOUND:xcom3/rawsound/tactical/aliens/cries/{}{}.raw:22050",
						    crySfxName, i)));
				else
					a->crySfx.push_back(fw().data->loadSample(fmt::format(
				...
				if (dieSfxName.length() > 0)
					a->dieSfx[AgentType::Gender::Male].push_back(fw().data->loadSample(
					    fmt::format("RAWSOUND:xcom3/rawsound/tactical/aliens/deaths/{}.raw:22050",
					                dieSfxName)));
				break;
			default:
				...
				for (int i = 1; i <= 3; i++)
				{
					a->damageSfx[AgentType::Gender::Male].push_back(fw().data->loadSample(
					    fmt::format("RAWSOUND:xcom3/rawsound/extra/hpainm{}.raw:22050", i)));
					a->fatalWoundSfx[AgentType::Gender::Male].push_back(fw().data->loadSample(
					    fmt::format("RAWSOUND:xcom3/rawsound/extra/hdethm{}.raw:22050", i)));
					a->dieSfx[AgentType::Gender::Male].push_back(fw().data->loadSample(
					    fmt::format("RAWSOUND:xcom3/rawsound/extra/hdethm{}.raw:22050", i)));
				}
				for (int i = 1; i <= 2; i++)
				{
					a->damageSfx[AgentType::Gender::Female].push_back(fw().data->loadSample(
					    fmt::format("RAWSOUND:xcom3/rawsound/extra/hpainf{}.raw:22050", i)));
					a->fatalWoundSfx[AgentType::Gender::Female].push_back(fw().data->loadSample(
					    fmt::format("RAWSOUND:xcom3/rawsound/extra/hdethf{}.raw:22050", i)));
		...
		a->damage_modifier = {
		    &state,
		    fmt::format("{}{}", DamageModifier::getPrefix(),
		...
		a->equipment_layout = {
		    &state, fmt::format("{}{}", AgentEquipmentLayout::getPrefix(), canon_string(name))};
		...
		switch (i)
		{
			...
			{
				auto liveName =
				    fmt::format("{}{}_ALIVE", AEquipmentType::getPrefix(), canon_string(a->name));
				auto deadName =
				    fmt::format("{}{}_DEAD", AEquipmentType::getPrefix(), canon_string(a->name));

				auto liveItem = mksp<AEquipmentType>();
				liveItem->bioStorage = true;
				liveItem->equipscreen_size = {6, 5};
				liveItem->equipscreen_sprite =
				    fw().data->loadImage(fmt::format("PCK:xcom3/ufodata/contico.pck:xcom3/ufodata/"
				...
				liveItem->name = fmt::format("Live {}", a->name);
				...
				deadItem->name = fmt::format("Dead {}", a->name);
				deadItem->score = 0;
	...
	{
		UString name = "NONE";
		UString id = fmt::format("{}{}", AgentEquipmentLayout::getPrefix(), canon_string(name));
...
void InitialGameStateExtractor::extractAgentBodyTypes(GameState &state) const
{
	...
	for (int i = 0; i <= 46; i++)
	{
		...
		UString id = fmt::format("{}{}", AgentBodyType::getPrefix(), canon_string(name));
									...
									for (int z = 0; z < 2; z++)
									{
										...
										for (int i = 0; i < limit; i++)
										{
											a
											    ...
											    ->setSlice(
											        i,
											        fw().data->loadVoxelSlice(fmt::format(
							...
							for (int fy = -1; fy <= 1; fy++)
							{
								...
								for (int i = 0; i < (a->height[entry.first]) / 2; i++)
								{
									a->voxelMaps[entry.first][facing][0]->setSlice(
									    i, fw().data->loadVoxelSlice(fmt::format(
