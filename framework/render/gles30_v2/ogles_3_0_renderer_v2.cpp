...
class Spritesheet
{
  ...
  public:
	...
	void repack()
	{
		...
		{
			LogInfo("Repack: creating sheet {0}", (int)pages.size());
...
class OGLES30Renderer final : public Renderer
{
  ...
  public:
	...
	{
		LogInfo("Max {0} sprite buffers {1} textured buffers {2} coloured buffers",
	...
	void newFrame() override
	{
		...
		{
			LogInfo("New max sprite buffers: {0}", this->spriteMachine->used_buffers);
			this->maxSpriteBuffers = this->spriteMachine->used_buffers;
		}
		if (this->texturedMachine->used_buffers > this->maxTexturedBuffers)
		{
			LogInfo("New max textured buffers: {0}", this->texturedMachine->used_buffers);
		...
		{
			LogInfo("New max coloured buffers: {0}", this->colouredDrawMachine->used_buffers);
                                          ...
                                          const GL::GLchar *message, const void *)
{
	switch (severity)
	{
		...
		{
			LogWarning("Debug message: \"{0}\"", message);
			break;
		}
		default:
		{
			LogInfo("Debug message: \"{0}\"", message);
			break;
		}
	}
}
...
OGLES30Renderer::OGLES30Renderer() : state(State::Idle)
{
	...
	LogInfo("Set spritesheet size to {0}", spritesheetPageSize);
