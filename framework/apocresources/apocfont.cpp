...
sp<BitmapFont> ApocalypseFont::loadFont(const UString &fontDescPath)
{
	...
	if (!file)
	{
		LogWarning("Failed to open font file at path \"{0}\"", fontDescPath);
		return nullptr;
	}
	...
	if (!data)
	{
		LogWarning("Failed to read font file at path \"{0}\"", fontDescPath);
		return nullptr;
	}
	...
	if (!parseResult)
	{
		LogWarning("Failed to parse font file at \"{0}\" - \"{1}\" at \"{2}\"", fontDescPath,
	...
	if (!openapocNode)
	{
		LogWarning("Failed to find \"openapoc\" root node in font file at \"{0}\"", fontDescPath);
		return nullptr;
	}
	...
	if (!fontNode)
	{
		LogWarning("Failed to find \"openapoc::apocfont\" node in font file at \"{0}\"",
		           fontDescPath);
		return nullptr;
	}
	...
	if (height <= 0)
	{
		LogError("apocfont \"{0}\" with invalid \"height\" attribute", fontName);
		return nullptr;
	}
	spacewidth = fontNode.attribute("spacewidth").as_int();
	if (spacewidth <= 0)
	{
		LogError("apocfont \"{0}\" with invalid \"spacewidth\" attribute", fontName);
		return nullptr;
	}
	kerning = fontNode.attribute("kerning").as_int();
	if (kerning <= 0)
	{
		LogError("apocfont \"{0}\" with invalid \"kerning\" attribute", fontName);
		return nullptr;
	}
	     ...
	     glyphNode = glyphNode.next_sibling("glyph"))
	{
		...
		{
			LogError("apocfont \"{0}\" glyph \"{1}\" has {2} codepoints, expected one - skipping "
			         "glyph",
		...
		{
			LogError("Font \"{0}\" has multiple glyphs for string \"{1}\" - skipping glyph",
