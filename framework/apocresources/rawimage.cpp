...
sp<PaletteImage> RawImage::load(Data &data, const UString &filename, const Vec2<int> &size)
{
	...
	if (!infile)
	{
		LogWarning("Failed to open file \"{0}\"", filename);
		return nullptr;
	}
	if (size.x <= 0 || size.y <= 0)
	{
		LogWarning("Trying to read image of invalid size {{{0},{1}}}", size.x, size.y);
		return nullptr;
	}

	if (infile.size() != static_cast<size_t>(size.x * size.y))
	{
		LogWarning("File \"{0}\" has incorrect size for raw image of size {1}", filename, size);
	}
	...
	for (int y = 0; y < size.y; y++)
	{
		for (int x = 0; x < size.x; x++)
		{
			...
			{
				LogError("Unexpected EOF in file \"{0}\" at {{{1},{2}}}", filename, x, y);
				return nullptr;
			}
...
sp<ImageSet> RawImage::loadSet(Data &data, const UString &filename, const Vec2<int> &size)
{
	...
	if (size.x <= 0 || size.y <= 0)
	{
		LogWarning("Trying to read images of invalid size {0}", size);
		return nullptr;
	}

	if (infile.size() % static_cast<size_t>(size.x * size.y) != 0)
	{
		LogWarning("File \"{0}\" has incorrect size for raw images of size {1}", filename, size);
	}
	...
	for (size_t i = 0; i < numImages; i++)
	{
		...
		for (int y = 0; y < size.y; y++)
		{
			for (int x = 0; x < size.x; x++)
			{
				...
				if (!infile.read(reinterpret_cast<char *>(&idx), 1))
				{
					LogError("Unexpected EOF in file \"{0}\" at {{{1}:{2},{3}}}", filename, i, x,
					         y);
					return nullptr;
				}
				l.set(Vec2<unsigned int>{x, y}, idx);
