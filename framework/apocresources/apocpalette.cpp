...
sp<Palette> loadPCXPalette(Data &data, const UString fileName)
{
	...
	{
		LogInfo("Skipping file \"%s\" as it doesn't look like a .pcx", fileName.cStr());
		return nullptr;
	}
	...
	if (!file)
	{
		LogInfo("File \"%s\" failed to be opened", fileName.cStr());
		return nullptr;
	}
	...
	{
		LogInfo("File \"%s\" has size %zu - too small for header and palette", fileName.cStr(),
		        file.size());
		return nullptr;
	}
	...
	if (!file)
	{
		LogInfo("File \"%s\" failed to read PCX header", fileName.cStr());
		return nullptr;
	}

	if (header.Identifier != PcxIdentifier)
	{
		LogInfo("File \"%s\" doesn't have PCX header magic", fileName.cStr());
		return nullptr;
	}

	if (header.BitsPerPixel != 8)
	{
		LogInfo("File \"%s\" has non-8-bit image", fileName.cStr());
		return nullptr;
	}
