  ...
  public:
	...
	void play(const std::vector<UString> &tracks, PlayMode mode) override
	{
		...
		for (auto &track : tracks)
		{
			...
			if (!musicTrack)
				LogError("Failed to load music track \"{0}\" - skipping", track);
			else
	...
	static void progressTrack(void *data)
	{
		...
		LogInfo("Playing track {0} ({1})", jukebox->position,
	...
	void loadPlaylists() override
	{
		...
		for (const auto &[playlist, path] : playlistPaths)
		{
			...
			if (!file)
			{
				LogWarning("Failed to open playlist file \"{0}\"", path);
				continue;
			}
			auto data = file.readAll();
			if (!data)
			{
				LogWarning("Failed to read playlist file \"{0}\"", path);
				continue;
			}
			...
			if (!result)
			{
				LogWarning("Failed to parse playlist \"{0}\" - \"{1}\" at \"{2}\"", path,
			...
			if (!node)
			{
				LogWarning("No root \"openapoc_playlist\" element in playlist file \"{0}\"", path);
				continue;
			}
