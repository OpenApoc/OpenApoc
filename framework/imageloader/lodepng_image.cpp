...
sp<Palette> OpenApoc::loadPNGPalette(Data &d, const UString fileName)
{
	...
	if (!f)
	{
		LogInfo("Failed to open file \"{0}\" - skipping", fileName);
		return nullptr;
	}
	auto data = f.readAll();
	...
	if (err)
	{
		LogInfo("Failed to read PNG headers from \"{0}\" ({1}) : {2} - skipping", f.systemPath(),
	...
	if (width * height != 256)
	{
		...
		LogWarning("PNG \"{0}\" size {{{1},{2}}} too large for palette (must be 256 pixels total)",
	...
	if (error)
	{
		LogInfo("Failed to read PNG \"{0}\" ({1}) : {2}", f.systemPath(), err,
...
class LodepngImageLoader : public OpenApoc::ImageLoader
{
  public:
	...
	sp<Image> loadImage(IFile &file) override
	{
		...
		if (err)
		{
			LogInfo("Failed to read PNG headers from \"{0}\" ({1}) : {2}", file.systemPath(), err,
			        lodepng_error_text(err));
			return nullptr;
		}

		LogInfo("Loading PNG \"{0}\" size {{{1},{2}}} - colour mode {3} depth {4}",
		        file.systemPath(), width, height,
		        static_cast<int>(png_state.info_png.color.colortype),
		...
		if (error)
		{
			LogInfo("LodePNG error code: {0}: {1}", error, lodepng_error_text(error));
		}
		if (!image.size())
		{
			LogInfo("Failed to load image {0} (not a PNG?)", file.systemPath());
			return nullptr;
		}
...
class LodepngImageWriter : public OpenApoc::ImageWriter
{
  public:
	...
	bool writeImage(sp<PaletteImage> img, std::ostream &outStream, sp<Palette> pal) override
	{
		...
		{
			LogWarning("Only 256 colour palettes supported (got {0})",
		...
		for (unsigned i = 0; i < 256; i++)
		{
			...
			if (err)
			{
				LogWarning("Failed to add palette index {0} to PNG: {1}: {2}", i, err,
				           lodepng_error_text(err));
				return false;
			}
		}
		...
		if (err)
		{
			LogWarning("Failed to encode PNG: {0}: {1}", err, lodepng_error_text(err));
			return false;
		}
		outStream.write(reinterpret_cast<char *>(outBuf.data()), outBuf.size());
		if (!outStream)
		{
			LogWarning("Failed to write {0} bytes to stream", outBuf.size());
			return false;
		}
