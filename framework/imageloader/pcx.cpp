...
class PCXImageLoader : public OpenApoc::ImageLoader
{
  public:
	...
	sp<OpenApoc::Image> loadImage(IFile &file) override
	{
		...
		{
			LogInfo("File \"%s\" had invalid PCX identifier 0x%02x", path.cStr(),
		...
		{
			LogWarning("File \"%s\" had invalid PCX Encoding 0x%02x", path.cStr(),
		...
		{
			LogWarning("File \"%s\" had invalid PCX BitsPerPixel 0x%02x", path.cStr(),
		...
		{
			LogWarning("File \"%s\" had invalid PCX NumBitPlanes 0x%02x", path.cStr(),
		...
		if (pal != 0x0C)
		{
			LogWarning("File \"%s\" had invalid PCX palette identifier byte 0x%02x", path.cStr(),
...

		{
			...
			for (unsigned y = header->YStart; y <= header->YEnd; y++)
			{
				...
				while (x <= header->XEnd)
				{
					...
					{
						LogWarning("Unexpected EOF reading PCX file \"%s\" ", path.cStr());
						return nullptr;
					}
					b = *img_data++;
