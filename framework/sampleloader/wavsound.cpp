...
class WavSampleLoader : public SampleLoader
{
  ...
  public:
	...
	sp<Sample> loadSample(UString path) override
	{
		...
		{
			LogInfo("String \"{0}\" doesn't look like a rawsample - need 2 elements (got {1})",
			        path, splitString.size());
			return nullptr;
		}
		if (splitString[0] != "WAV")
		{
			LogInfo("String \"{0}\" doesn't look like a wav - no WAV prefix", path);
			return nullptr;
		}
		const auto file = data.fs.open(splitString[1]);
		if (!file)
		{
			LogWarning("wav \"{0}\" failed to open file \"{1}\"", path, splitString[1]);
			return nullptr;
		}
		...
		{
			LogWarning("Failed to open WAV file at \"{0}\" - \"{1}\"", fullPath, SDL_GetError());
			return nullptr;
		}

		if (spec.channels != 1)
		{
			LogWarning("Failed to open WAV file at \"{0}\" - only single channel samples supported",
			           fullPath);
			SDL_FreeWAV(buf);
			return nullptr;
		}
		...
		if (!format)
		{
			LogWarning("Failed to open WAV file at \"{0}\" - unsupported SDL format {1:08x}",
