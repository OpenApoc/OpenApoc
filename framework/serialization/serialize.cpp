...
SerializationNode *SerializationNode::getNodeReq(const char *name)
{
	...
	if (!node)
	{
		throw SerializationException(format("Missing node \"{0}\"", name), this);
	}
	return node;
}
...
SerializationNode *SerializationNode::getSectionReq(const char *name)
{
	...
	if (!node)
	{
		throw SerializationException(format("Missing section \"{0}\"", name), this);
	}
	return node;
}
...
SerializationNode *SerializationNode::getNextSiblingReq(const char *name)
{
	...
	if (!node)
	{
		throw SerializationException(format("Missing sibling of \"{0}\"", name), this);
	}
	return node;
}
...
up<SerializationArchive> SerializationArchive::readArchive(const UString &path)
{
	...
	{
		LogWarning("Failed to open archive at \"{0}\"", path);
		return nullptr;
	}
	LogInfo("Opened archive \"{0}\"", path);
...
SerializationNode *XMLSerializationArchive::getRoot(const UString &prefix, const char *name)
{
	...
	{
		LogWarning("Reading from not opened archive: {0}!", path);
		return nullptr;
	}
	...
	if (!root)
	{
		LogWarning("Failed to find root with name \"{0}\" in \"{1}\"", name, path);
		return nullptr;
	}
	const UString *newPrefix = &this->prefixes.emplace_back(prefix + name + "/");
	return &this->nodes.emplace_back(this, root, newPrefix);
...
unsigned char XMLSerializationNode::getValueUChar()
{
	...
	if (uint > std::numeric_limits<unsigned char>::max())
	{
		throw SerializationException(
		    format("Value {0} is out of range of unsigned char type", uint), this);
	}
...
std::vector<bool> XMLSerializationNode::getValueBoolVector()
{
	...
	for (size_t i = 0; i < string.length(); i++)
	{
		...
		if (c == '1')
			...
			throw SerializationException(format("Unknown char '{0}' in bool vector", c), this);
	}
	return vec;
}
