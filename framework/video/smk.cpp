...
class SMKVideo : public Video, public std::enable_shared_from_this<SMKVideo>
{
  public:
	...
	bool readNextFrame()
	{
		...
		if (ret == SMK_ERROR)
		{
			LogWarning("Error decoding frame {0}", this->current_frame_read);
			return false;
		}

		if (ret == SMK_LAST)
		{
			LogInfo("Last frame {0}", this->current_frame_read);
			this->stopped = true;
		}
		...
		if (!palette_data)
		{
			LogWarning("Failed to get palette data for frame {0}", this->current_frame_read);
			return false;
		}
		const unsigned char *image_data = smk_get_video(this->smk_ctx);
		if (!image_data)
		{
			LogWarning("Failed to get image data for frame {0}", this->current_frame_read);
			return false;
		}
		...
		if (audio_bytes == 0)
		{
			LogWarning("Error reading audio size for frame {0}", this->current_frame_read);
			return false;
		}
		...
		if (!sample_pointer)
		{
			LogWarning("Error reading audio data for frame {0}", this->current_frame_read);
			return false;
		}
		memcpy(audio_frame->samples.get(), sample_pointer, audio_bytes);

		LogInfo("Read {0} samples bytes, {1} samples", audio_bytes, audio_frame->sample_count);

		this->image_queue.push(frame);
		this->audio_queue.push(audio_frame);

		LogInfo("read frame {0}", this->current_frame_read);
	...
	bool load(IFile &file)
	{
		...
		LogInfo("Read {0} bytes from video",
		...
		if (!this->smk_ctx)
		{
			LogWarning("Failed to read SMK file \"{0}\"", video_path);
		...
		{
			LogWarning("Failed to read SMK file info from \"{0}\"", video_path);
			this->video_data.reset();
		...
		LogInfo("Video frame count {0}, ns per frame = {1} (USF: {2})", this->frame_count,
		        this->frame_time.count(), usf);

		unsigned long height, width;
		if (smk_info_video(this->smk_ctx, &width, &height, nullptr))
		{
			LogWarning("Failed to read SMK video info from \"{0}\"", video_path);
			this->video_data.reset();
			smk_close(this->smk_ctx);
			this->smk_ctx = nullptr;
			return false;
		...
		LogInfo("Video frame size {{{0},{1}}}", this->frame_size.x, this->frame_size.y);

		auto ret = smk_enable_video(this->smk_ctx, 1);
		if (ret == SMK_ERROR)
		{
			LogWarning("Error enabling video for \"{0}\"", video_path);
			return false;
		}
		...
		if (ret == SMK_ERROR)
		{
			LogWarning("Error reading audio info for \"{0}\"", video_path);
			return false;
		}
		...
		if (audio_track_mask & SMK_AUDIO_TRACK_0)
		{
			...
			LogInfo("Audio track: channels {0} depth {1} rate {2}", (unsigned)channels[0],
			        (unsigned)bitdepth[0], audio_rate[0]);
		}
		else
		{
			LogWarning("Unsupported audio track mask {0:02x} for \"{1}\"",
		...
		switch (channels[0])
		{
			...
			case 2:
				...
				break;
			default:
				LogWarning("Unsupported audio channel count {0} for \"{1}\"", (unsigned)channels[0],
				           video_path);
				return false;
		}
		switch (bitdepth[0])
		{
			...
			case 16:
				...
				break;
			default:
				LogWarning("Unsupported audio bit depth {0} for \"{1}\"", (unsigned)bitdepth[0],
				           video_path);
				return false;
		}
		...
		if (ret == SMK_ERROR)
		{
			LogWarning("Error enabling audio track 0 for \"{0}\"", video_path);
		}
