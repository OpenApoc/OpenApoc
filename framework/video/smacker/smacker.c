...
static char smk_render_palette(struct smk_video_t * s, unsigned char * p, unsigned long size)
{
	...
	while ((i < 256) && (size > 0)) {
		if ((*p) & 0x80) {
			...
			/* OpenApoc note: this vendored copy already carries upstream commit
			   acad48d ("Remove spurious overlap check in palette_render 0x40
			   handler"), which is the fix the fork's dependencies/libsmacker used to
			   carry as a local 1-line patch (removing an `(src < i && src+count > i)`
			   overlap guard). Without it, the XCom:Apocalypse intro video's overlapping
			   palette copies get rejected and playback shows corrupted colours. Nothing
			   left to patch here - kept as a marker so this isn't mistaken for stock
			   upstream behavior if it's ever re-synced. */
