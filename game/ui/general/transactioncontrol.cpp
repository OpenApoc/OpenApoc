...
void TransactionControl::updateValues()
{
	...
	stockLeft->setText(format("{0}", tradeState.getLeftStock(true)));
	stockRight->setText(format("{0}", tradeState.getRightStock(true)));
	deltaLeft->setText(format("{0}{1}", curDeltaLeft > 0 ? "+" : "", curDeltaLeft));
	deltaRight->setText(format("{0}{1}", curDeltaRight > 0 ? "+" : "", curDeltaRight));
                                                         ...
                                                         int indexLeft, int indexRight)
{
	...
	switch (agent->type->role)
	{
		...
		case AgentType::Role::Soldier:
			...
			break;
		default:
			LogError("Unknown type of agent {0}.", agent.id);
			return nullptr;
	}
                                                         ...
                                                         int indexRight)
{
	...
	LogInfo("Vehicle type {0} starting price {1}", vehicle->type.id, price);
	...
	for (auto &e : vehicle->equipment)
	{
		if (state.economy.find(e->type.id) != state.economy.end())
		{
			...
			LogInfo("Vehicle type {0} price increased to {1} after counting {2}", vehicle->type.id,
			        price, e->type.id);
		}
	}
	...
	for (auto &e : vehicle->type->initial_equipment_list)
	{
		if (state.economy.find(e.second.id) != state.economy.end())
		{
			...
			LogInfo("Vehicle type {0} price decreased to {1} after counting {2}", vehicle->type.id,
			        price, e.second.id);
		}
	}
	LogInfo("Vehicle type {0} final price {1}", vehicle->type.id, price);
                                  ...
                                  std::vector<int> &initialStock, int indexLeft, int indexRight)
{
	...
	// Name
	const UString &labelName = researched ? name : tr("Alien Artifact");
	...
	{
		auto label = control->createChild<Label>(control->manufacturerName, labelFont);
		if (manufacturerHostile || manufacturerUnavailable)
		{
