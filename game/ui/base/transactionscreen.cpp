...
void TransactionScreen::displayItem(sp<TransactionControl> control)
{
	 ...
	 || control->itemType == TransactionControl::Type::AgentEquipmentCargo)
	{
		...
		formItemAgent->findControlTyped<Graphic>("SELECTED_IMAGE")->setImage(agentEquipment->equipscreen_sprite);
		...
		switch(agentEquipment->type)
		{
			...
			case AEquipmentType::Type::Weapon:
			{
				...
				if (agentEquipment->damage_type)
				{
					...
					labels['R'][2]->setText(format("%.2f", (float)TICKS_PER_SECOND / (float)agentEquipment->fire_delay));
					labels['R'][3]->setText(format("%d", agentEquipment->range / (int)VELOCITY_SCALE_BATTLE.x));
				...
				else
				{
					...
					{
						labels['R'][1]->setText(format("%d", agentEquipment->ammo_types.front()->accuracy));
						labels['R'][2]->setText(format("%.2f", (float)TICKS_PER_SECOND / (float)agentEquipment->ammo_types.front()->fire_delay));
						labels['R'][3]->setText(format("%d", agentEquipment->ammo_types.front()->range));
					}
				}
			}
			break;
	      ...
	      || control->itemType == TransactionControl::Type::VehicleAmmo)
	{
		...
		if (control->itemType == TransactionControl::Type::VehicleType)
		{
			...
			formItemVehicle->findControlTyped<Label>("ITEM_NAME")
				->setText(vehicleType->name);
		...
		else if (control->itemType == TransactionControl::Type::Vehicle)
		{
			...
			{
				statsValues[0]->setText(format("%d/%d", vehicle->getConstitution(), vehicle->getMaxConstitution()));
			}
			...
			statsValues[7]->setText(format("%d/%d", vehicle->getPassengers(), vehicle->getMaxPassengers()));
		}
		else if (control->itemType == TransactionControl::Type::VehicleEquipment)
		{
			...
			switch (vehicleEquipment->type)
			{
				...
				case EquipmentSlotType::VehicleGeneral:
				{
					...
					if (vehicleEquipment->cargo_space)
					{
						...
						statsValues[statsCount]->setText(format("%d", vehicleEquipment->cargo_space));
						statsCount++;
					}
					if (vehicleEquipment->passengers)
					{
						statsLabels[statsCount]->setText(tr("Passengers"));
						statsValues[statsCount]->setText(format("%d", vehicleEquipment->passengers));
						statsCount++;
					}
					if (vehicleEquipment->alien_space)
					{
						statsLabels[statsCount]->setText(tr("Aliens Held"));
						statsValues[statsCount]->setText(format("%d", vehicleEquipment->alien_space));
						statsCount++;
					}
					if (vehicleEquipment->missile_jamming)
					{
						statsLabels[statsCount]->setText(tr("Jamming"));
						statsValues[statsCount]->setText(format("%d", vehicleEquipment->missile_jamming));
						statsCount++;
					}
