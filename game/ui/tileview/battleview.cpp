...
void BattleView::updateSoldierButtons()
{
	...
	//In turnbased : If a single unit is selected and it has active weapon - show TU cost for aim types in tooltips
	if ((battle.mode == Battle::Mode::TurnBased) && (battle.battleViewSelectedUnits.size() == 1) &&
		(selectionState == BattleSelectionState::FireLeft || selectionState == BattleSelectionState::FireRight))
	{
