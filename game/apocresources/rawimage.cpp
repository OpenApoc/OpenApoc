...
RawImage::load(Data& data, const UString &filename, const Vec2<int> &size)
{
	...
	{
		LogWarning("Trying to read image of invalid size {%d,%d}",
			size.x, size.y);
		return nullptr;
	}
	...
	for (int y = 0; y < size.y; y++)
	{
		for (int x = 0; x < size.x; x++)
		{
			...
			{
				LogError("Unexpected EOF in file \"%s\" at {%d,%d}",
					filename.str().c_str(), x, y);
				return nullptr;
			}
