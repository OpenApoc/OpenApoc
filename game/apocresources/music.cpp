...
public:
	RawMusicTrack(Data &data, const UString &fileName, unsigned int fileOffset, unsigned int numSamples)
		: file(data.load_file(fileName)), valid(false)
	{
		if (!file)
		{
		...
		{
			LogError("File \"%s\" insufficient size for offset %u + size %zu - returned size %zu", fileName.str().c_str(), fileOffset, numSamples * MusicChannels * MusicBytesPerSample, file.size());
			return;
		}
...
public:
	...
	virtual std::shared_ptr<MusicTrack> loadMusic(UString path)
	{
		...
		return std::make_shared<RawMusicTrack>(*fw.data, strings[0], starts[track], lengths[track]/MusicChannels/MusicBytesPerSample);
	}
