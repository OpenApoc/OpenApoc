...
public:
	...
	virtual std::shared_ptr<Image> getSprite() override;
};
...
public:
	TileObjectDirectionalSprite(TileMap &map, Vec3<float> position, std::vector<std::pair<Vec3<float>, std::shared_ptr<Image>>> sprites, Vec3<float> initialDirection = Vec3<float>{1,0,0});
	virtual std::shared_ptr<Image> getSprite() override;
