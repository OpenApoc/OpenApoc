...
const Vec3<int> &TileObjectCollidable::getTileSizeInVoxels() const
{
	return this->tileSizeInVoxels;
}

const Vec3<int> &TileObjectCollidable::getBounds() const { return this->bounds; }

bool TileObjectCollidable::hasVoxelAt(const Vec3<float> &worldPosition) const
{
...
void TileObjectCollidable::handleCollision(const Collision &c)
{
...
void TileObjectCollidable::setPosition(Vec3<float> newPos)
{
...
void TileObjectCollidable::removeFromAffectedTiles()
{
...
void TileObjectCollidable::addToAffectedTiles()
{
	auto thisPtr = std::dynamic_pointer_cast<TileObjectCollidable>(shared_from_this());
	if (!thisPtr) {
		LogError("this not a TileObjectCollidable?");
