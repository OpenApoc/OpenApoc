...
namespace OpenApoc
{
	...
	bool
	RulesLoader::ParseOrganisationDefinition(Framework &fw, Rules &rules, tinyxml2::XMLElement *root)
	{
		std::ignore = fw;
		...
		LogInfo("Organisation \"%s\" at idx %d", def.name.str().c_str(),
			rules.organisations.size());
