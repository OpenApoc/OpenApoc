...
bool operator==(const UnitAI &a, const UnitAI &b)
{
	...
	LogError("Unsupported comparison for UserAI type {0}", (int)a.type);
	return false;
}
...
bool operator==(const TacticalAI &a, const TacticalAI &b)
{
	...
	LogError("Unsupported comparison for Tactical type {0}", (int)a.type);
	return false;
}
...
bool GameState::serialize(SerializationArchive *archive) const
{
	...
	{
		LogError("Serialization failed: \"{0}\"", e.what());
		return false;
	}
	return true;
}
...
bool BattleUnitImagePack::loadImagePack(GameState &state, const UString &path)
{
	...
	if (!file)
	{
		LogError("Failed to open image pack \"{0}\"", path);
		return false;
	}
	...
	if (!archive)
	{
		LogError("Failed to read \"{0}\"", fullPath);
		return false;
	}
...
bool BattleUnitAnimationPack::loadAnimationPack(GameState &state, const UString &path)
{
	...
	if (!file)
	{
		LogError("Failed to open animation pack \"{0}\"", path);
	}
