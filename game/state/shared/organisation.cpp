                                                     ...
                                                     const StateRef<Building> &buyer) const
{
	...
	else
	{
		...
		return pick_random(state.rng, purchaseBuildings);
	}
}
...
void Organisation::updateHirableAgents(GameState &state)
{
	...
	if (state.getCivilian().id == id)
	{
		...
		hireeLocation = pick_random(state.rng, buildingsWithoutBases);
	}
	else
	{
		if (buildings.empty())
		...
		hireeLocation = pick_random(state.rng, buildings);
	}
...
void Organisation::updateVehicleAgentPark(GameState &state)
{
	...
	//	}
	//	sp<Building> building = pick_random(state.rng, buildingsRandomizer);
	...
	for (auto &entry : vehiclePark)
	{
		...
		while (countVehicles < entry.second)
		{
			...
			StateRef<Building> building = pick_random(state.rng, buildingsRandomizer);
                                    ...
                                    StateRef<Organisation> owner)
{
	...
	if (pattern.target == Organisation::MissionPattern::Target::ArriveFromSpace)
	{
		...
		auto linerType = pick_random(state.rng, pattern.allowedTypes);
		auto liner = city->placeVehicle(state, linerType, owner,
		                                VehicleMission::getRandomMapEdgeCoordinates(state, city));

		auto building = pick_random(state.rng, spaceports);
	...
	auto sourceBuilding = pick_random(state.rng, buildingsRandomizer);
	...
	auto destBuilding = pick_random(state.rng, buildingsRandomizer);
