                                ...
                                StateRef<AEquipmentType> item)
{
	...
	if (success)
	{
		fw().soundBackend->playSample(pick_random(state.rng, *psiSuccessSounds), position);
		if (!realTime)
		{
	...
	{
		fw().soundBackend->playSample(pick_random(state.rng, *psiFailSounds), position);
		return false;
	}
}
                                   ...
                                   StateRef<BattleUnit> attacker, bool violent)
{
	...
	if (wasConscious)
	{
		...
		if (fatal)
		{
			    ...
			    !agent->type->fatalWoundSfx.at(agent->gender).empty())
			{
				fw().soundBackend->playSample(
				    pick_random(state.rng, agent->type->fatalWoundSfx.at(agent->gender)), position);
			}
		}
		// Emit sound wound
		else if (stunPower == 0)
		{
			...
			{
				fw().soundBackend->playSample(
				    pick_random(state.rng, agent->type->damageSfx.at(agent->gender)), position);
			}
		}
	}
                             ...
                             BodyPart bodyPart, DamageSource source, StateRef<BattleUnit> attacker)
{
	...
	{
		fw().soundBackend->playSample(pick_random(state.rng, *genericHitSounds), position);
	}
	...
	else if (damageType->effectType == DamageType::EffectType::Fire)
	{
		switch (source)
		{
			case DamageSource::Impact:
			{
				...
				damage = pick_random(state.rng, damageDistribution);
				break;
			}
...
void BattleUnit::updateCrying(GameState &state)
{
	...
	if (ticksUntillNextCry == 0)
	{
		...
		// Actually cry
		fw().soundBackend->playSample(pick_random(state.rng, agent->type->crySfx), getPosition());
	}
}
...
void BattleUnit::applyEnzymeEffect(GameState &state)
{
	...
	{
		auto item = pick_random(state.rng, agent->equipment);
...
void BattleUnit::die(GameState &state, StateRef<BattleUnit> attacker, bool violently)
{
	...
	{
		fw().soundBackend->playSample(pick_random(state.rng, agent->type->dieSfx.at(agent->gender)),
		                              position);
	}
	// Morale and score
