...
BattleExplosion::BattleExplosion(Vec3<int> position, StateRef<DamageType> damageType, int power, int depletionRate, StateRef<BattleUnit> ownerUnit) :
	position(position), ticksUntilExpansion(TICKS_MULTIPLIER * 2), locationsToExpand({{position, power}}), locationsVisited({ position }), damageType(damageType), depletionRate(depletionRate), ownerUnit(ownerUnit) {}
...
void BattleExplosion::damage(GameState &state, const TileMap &map, Vec3<int> pos, int power)
{
	...
	if (!damageType->gas)
	{
		for (auto obj : tile->ownedObjects)
		{
			...
			{
				StateRef<BattleUnit> u = { &state, std::static_pointer_cast<TileObjectBattleUnit>(obj)->getUnit()->id };
				...
				else
				{
					...
					u->applyDamage(state, power, damageType, u->determineBodyPartHit(damageType, cposition, velocity));
				}
			}
...
void BattleExplosion::expand(GameState &state, const TileMap &map, const Vec3<int> &from, const Vec3<int> &to, int power)
{
	// list of coordinates to check
	static const std::map<Vec3<int>, std::list<std::pair<Vec3<int>, std::set<TileObject::Type>>>> searchPattern = {
		// Vertical
		{ { 0, 0, 1 },{ { { 0, 0, 1 },{ TileObject::Type::Ground, TileObject::Type::Feature } } } },
		{ { 0, 0, -1 },{ { { 0, 0, 0 },{ TileObject::Type::Ground } } ,{ { 0, 0, -1 },{ TileObject::Type::Feature } } } },
		// Horizontal direct
		{ { 0, -1, 0 },{ { { 0, 0, 0 },{ TileObject::Type::RightWall } } ,{ { 0, -1, 0 },{ TileObject::Type::Feature } } } },
		{ { 0, 1, 0 },{ { { 0, 1, 0 },{ TileObject::Type::RightWall } } ,{ { 0, 1, 0 },{ TileObject::Type::Feature } } } },
		{ { -1, 0, 0 },{ { { 0, 0, 0 },{ TileObject::Type::LeftWall } } ,{ { -1, 0, 0 },{ TileObject::Type::Feature } } } },
		{ { 1, 0, 0 },{ { { 1, 0, 0 },{ TileObject::Type::LeftWall } } ,{ { 1, 0, 0 },{ TileObject::Type::Feature } } } },
		// Horizontal top-left
		{ { -1, -1, 0 },{ { { -1, -1, 0 },{ TileObject::Type::Feature } },
	...
	depletion += depletionRate * (1 + (dir.x != 0 ? 1 : 0) + (dir.y != 0 ? 1 : 0) + (dir.z != 0 ? 1 : 0)) / 2;
