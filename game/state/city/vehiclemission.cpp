...
{
	auto &map = *state.cities[v.city.id]->map;
	...
	while (true)
	{
		...
		LogInfo("Cannot move to %d %d %d, contains scenery that is not a landing pad", target.x, target.y, target.z);
		target.z++;
		if (target.z >= map.size.z)
		{
			LogError("No space in the sky? Reached %d %d %d", target.x, target.y,
				target.z);
			return nullptr;
		}
	...
	// If it does, find a random location around it in a 5x5x5 cube that is not blocked
	...
	for (int i = 0; i < iterationCount; i++)
	{
		...
		bool containsScenery = false;
		for (auto obj : newTo->ownedObjects)
		{
			if (obj->getType() == TileObject::Type::Vehicle)
			{
		...
		if (!containsVehicle && !containsScenery)
		{
			foundNewTo = true;
			break;
		}
		newTarget = Vec3<int>{ 
			target.x + randBoundsInclusive(state.rng, -std::min(2, target.x),
				std::min(2, map.size.x - 1 - target.x)),
			target.y + randBoundsInclusive(state.rng, -std::min(2, target.y),
				std::min(2, map.size.y - 1 - target.y)),
			randBoundsInclusive(state.rng, std::min(1, (int)v.altitude - 3),
				std::min(3, map.size.z - 1 - (int)v.altitude))};

		newTo = map.getTile(newTarget);
	}
	if (foundNewTo && target != newTarget)
	{
		LogWarning("Target %d,%d,%d was unreachable, found new target %d,%d,%d", target.x, target.y, target.z, newTarget.x, newTarget.y, newTarget.z);
		target = newTarget;
	}
...
VehicleMission *VehicleMission::infiltrateBuilding(GameState &, Vehicle &, StateRef<Building> target)
{
	auto *mission = new VehicleMission();
...
bool VehicleMission::takeOffCheck(GameState &state, Vehicle &v, UString mission)
{
	if (!v.tileObject)
	{
		if (v.currentlyLandedBuilding)
		{
			LogInfo("Mission %s: Taking off first", mission.cStr());
	...
	switch (this->type)
	{
		case MissionType::TakeOff:
		{
			...
			for (auto &c : state.cities)
			{
				...
				{
					LogInfo("Taking off from building \"%s\" in city \"%s\"", b.id.cStr(),
					        c.first.cStr());
					city = {&state, c.first};
					break;
				}
			...
			if (!city)
			{
				LogError("No city found containing building \"%s\"", b.id.cStr());
				return;
			}
			auto &map = *city->map;
			for (auto padLocation : b->landingPadLocations)
			{
				...
				if (!tileHelper.canEnterTile(nullptr, padTile) ||
					...
					continue;
				LogInfo("Launching vehicle from building \"%s\" at pad {%d,%d,%d}", b.id.cStr(),
				        padLocation.x, padLocation.y, padLocation.z);
				this->currentPlannedPath = {padLocation, abovePadLocation};
				v.launch(map, state, padLocation);
				return;
			}
			LogInfo("No pad in building \"%s\" free - waiting", b.id.cStr());
			return;
		}
		case MissionType::Patrol:
		{
		...
		case MissionType::Land:
			if (finished && v.tileObject)
			{
				...
				v.land(state, b);
				LogInfo("Vehicle mission: Landed in %s", b.id.cStr());
				return;
			}
			return;
		case MissionType::Crash:
...
bool VehicleMission::isFinishedInternal(GameState &state, Vehicle &v)
{
	switch (this->type)
	{
		...
		{	
			auto vTile = v.tileObject;
			if (vTile && vTile->getOwningTile()->position == this->targetLocation)
				return true;
			return false;
		}
		case MissionType::GotoLocation:
...
void VehicleMission::start(GameState &state, Vehicle &v)
{
	switch (this->type)
	{
		...
		case MissionType::Land:
		{
			...
			if (!padFound)
			{
				LogError("Vehicle at {%d,%d,%d} not directly above a landing pad for building %s",
				         padPosition.x, padPosition.y, padPosition.z + 1, b.id.cStr());
				return;
			}
			this->currentPlannedPath = { padPosition, padPosition };
			return;
		}
		case MissionType::GotoPortal:
		{
			...
			{
				LogInfo("Vehicle mission %s: Pathing to portal at {%d,%d,%d}", getName().cStr(),
		...
		{
			auto name = this->getName();
			LogInfo("Vehicle mission %s checking state", name.cStr());
			auto t = this->targetVehicle;
			if (!t)
			{
				LogError("Target disappeared");
			...
			{
				LogError("Vehicle mission %s: Targeting itself", name.cStr());
				return;
			}
			auto targetTile = t->tileObject;
			if (!targetTile)
			{
				LogInfo("Vehicle mission %s: Target not on the map", name.cStr());
				return;
			}
			auto vehicleTile = v.tileObject;
		...
		case MissionType::GotoBuilding:
		{
			...
			{
				LogInfo("Vehicle mission %s: Already at building", name.cStr());
				return;
			}
			auto vehicleTile = v.tileObject;
			if (takeOffCheck(state, v, name))
			...
			LogInfo("Vehicle mission %s: at position {%d,%d,%d}", name.cStr(), position.x,
			...
			for (auto padLocation : b->landingPadLocations)
			{
				...
				{
					LogInfo("Mission %s: Landing on pad {%d,%d,%d}", name.cStr(), padLocation.x,
			...
			LogInfo("Vehicle mission %s: Pathing to pad at {%d,%d,%d}", name.cStr(),
...
void VehicleMission::setPathTo(Vehicle &v, Vec3<int> target, int maxIterations)
{
	...
	{
		LogError("Mission %s: Take off before pathfinding!", this->getName().cStr());
	}
}
...
bool VehicleMission::advanceAlongPath(GameState &state, Vec3<float> &dest, Vehicle &v)
{
	...
	// Land mission does not check for collision
