...
{
	template <>
	sp<BattleTileType> StateObject<BattleTileType>::get(const GameState &state, const UString &id)
	{
		
		auto it = state.battle.tile_types.find(id);
		if (it != state.battle.tile_types.end())
			return it->second;
	...
	template <> const UString &StateObject<BattleTileType>::getPrefix()
	{
		static UString prefix = "BATTLETILE_";
		return prefix;
	}
	template <> const UString &StateObject<BattleTileType>::getTypeName()
	{
		static UString name = "BattleTileType";
		return name;
	}
