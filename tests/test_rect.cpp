...
void test_point_within(Rect<int> r, Vec2<int> p, bool expected)
{
	...
	{
		LogError("Point {%d,%d} incorrectly %s rect {%d,%d},{%d,%d}",
			p.x, p.y, expected ? "not within":"within",
...
void test_rect_within(Rect<int> r1, Rect<int> r2, bool expected)
{
	...
	{
		LogError("Rect {%d,%d},{%d,%d} incorrectly %s rect {%d,%d},{%d,%d}",
			r2.p0.x, r2.p0.y, r2.p1.x, r2.p1.y,
			expected ? "not within":"within",
			r1.p0.x, r1.p0.y, r1.p1.x, r1.p1.y);
		exit(EXIT_FAILURE);
	}
}
void test_rect_intersects(Rect<int> r1, Rect<int> r2, bool expected)
{
	if (r1.intersects(r2) != expected)
	{
		LogError("Rect {%d,%d},{%d,%d} incorrectly %s rect {%d,%d},{%d,%d}",
			r2.p0.x, r2.p0.y, r2.p1.x, r2.p1.y,
			expected ? "does not intersect":"intersects",
			r1.p0.x, r1.p0.y, r1.p1.x, r1.p1.y);
		exit(EXIT_FAILURE);
	}
}
