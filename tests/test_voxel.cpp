...
static void check_voxel(Vec3<int> position, VoxelMap &v, bool expected)
{
	...
	{
		LogError("Unexpected voxel at {0} - expected {1}", position, expected ? 1 : 0);
		exit(EXIT_FAILURE);
	}
}
...
static void test_voxel(Vec3<int> voxel_size)
{
	...
	{
		LogError("Unexpected size {0}", v.size);
		exit(EXIT_FAILURE);
	}
	...
	{
		LogError("Unexpected centre {0} for empty map", v.getCentre());
		exit(EXIT_FAILURE);
	}
	...
	{
		LogError("Unexpected slice size {0}", slice->size);
		exit(EXIT_FAILURE);
	}
	// Ensure everything is '0' at init, and anything outside the bounds should be '0' too
	for (int y = -64; y < voxel_size.y + 64; y++)
	...
	if (bit_position.x > voxel_size.x)
	{
		...
		LogInfo("Clamping bit position x to {0}", bit_position.x);
	}
	if (bit_position.y >= voxel_size.y)
	{
		bit_position.y = voxel_size.y - 1;
		LogInfo("Clamping bit position y to {0}", bit_position.y);
	}
	...
	if (bit_voxel_position.z >= voxel_size.z)
	{
		...
		LogInfo("Clamping bit position z to {0}", bit_voxel_position.z);
	}
...
// The centre of a voxelmap with a single bit should be the same as that bit position
	...
	{
		LogError("Unexpected centre {0} for single-bit map, expected {1}", v.getCentre(),
		         bit_voxel_position);
		exit(EXIT_FAILURE);
	}
#endif
	...
	if (v.getCentre() != v.size / 2)
	{
		LogError("Unexpected centre {0} for reset-to-empty map", v.getCentre());
		exit(EXIT_FAILURE);
	}
#endif
	...
	if (bit_2_voxel_position.x < 0)
	{
		...
		LogInfo("Clamping bit 2 position x to {0}", bit_2_voxel_position.x);
	}
	if (bit_2_voxel_position.y < 0)
	{
		bit_2_voxel_position.y = 0;
		LogInfo("Clamping bit 2 position y to {0}", bit_2_voxel_position.y);
	}
	if (bit_2_voxel_position.z < 0)
	{
		bit_2_voxel_position.z = 0;
		LogInfo("Clamping bit 2 position z to {0}", bit_2_voxel_position.z);
	}
...
// Now check the centre
	...
	{
		LogError("Unexpected centre {0} for 2 bit map, expected {1}", v.getCentre(),
		         expected_centre);
		exit(EXIT_FAILURE);
	}
#endif
...
int main(int argc, char **argv)
{
	...
	{
		LogInfo("Testing voxel size {0}", size);
		test_voxel(size);
	}
}
