...
static bool test_32_8_roundtrip(const UString &str)
{
	...
	if (u8str != str)
	{
		LogError("String \"{0}\" ended up as \"{1}\" after utf8->utf32->utf8 roundtrip", str,
		         u8str);
		return false;
	}
	return true;
...
struct example_unicode
{
	...
	bool test() const
	{
		LogInfo("Testing string \"{0}\"", u8string);
		...
		{
			LogError("String \"{0}\" has unexpected length {1} , expected {2}", u8string,
		...
		{
			LogError("String \"{0}\" has unexpected iterated length {1} , expected {2}", u8string,
		...
		for (size_t i = 0; i < num_codepoints; i++)
		{
			...
			{
				LogError("String \"{0}\" has unexpected codepoint at index {1} - got 0x{2:x} "
				         "expected 0x{3:x}",
		...
		{
			LogError(
			    "String \"{0}\" compared false after utf8->unichar->utf8 conversion - got \"{1}\"",
                        ...
                        size_t count)
{
	...
	if (removed != expected)
	{
		LogError("\"{0}\".remove({1}, {2}) = \"{3}\", expected \"{4}\"", initial, offset, count,
		         removed, expected);
		return false;
	}
	return true;
                        ...
                        const UString &str)
{
	...
	{
		LogError("\"{0}\".inserted({1}, \"{2}\") = \"{3}\", expected \"{4}\"", initial, offset, str,
		         inserted, expected);
		return false;
	}
	return true;
...
int main(int argc, char **argv)
{
	...
	{
		LogError("toLower(\"{0}\") returned \"{1}\", expected \"{2}\"", example, lower,
		         lower_example);
		return EXIT_FAILURE;
	}
	if (upper != upper_example)
	{
		LogError("toUpper(\"{0}\") returned \"{1}\", expected \"{2}\"", example, upper,
		         upper_example);
		return EXIT_FAILURE;
	}
