...
template <typename T>
Vec2<T> Line<T>::GetIntersection( const Line<T> IntersectsWith )
{
	T lx[2];
	T ly[2];
	T ix[2];
...
template <typename T>
Vec2<T> Line<T>::ToVector()
{
	return new Vec2<T>( Points[1].X - Points[0].X, Points[1].Y - Points[0].Y );
}

template <typename T>
Angle<T> Line<T>::ToAngle()
{
	return Angle<T>( Points[0].AngleTo( Points[1] ) );
}

template <typename T>
Angle<T> Line<T>::Reflection( const Line<T> Projection )
{
...
template <typename T>
Vec2<T> Line<T>::GetSegmentPoint( int SegmentNumber, int NumberOfSegments )
{
